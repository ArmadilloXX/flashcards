...
module ApiFlashcards
  ...
  RSpec.shared_examples "unprocessable entity response" do |card_params|
    @original_text = card_params.fetch(:original_text, "")
    @translated_text = card_params.fetch(:translated_text, "")
    @block_id = card_params.fetch(:block_id, nil)

    describe "original_text=\'#{@original_text}\', "\
    "translated_text = \'#{@translated_text}\', "\
    "block_id = #{@block_id}" do
