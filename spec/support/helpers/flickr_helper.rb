module FlickrHelper
  def stub_flickr_requests
    recent_response = { "photos": {
      ...
      "photo": [
          { "id": "22026223630",
            ...
            "server": "600",
            "farm": 1,
            "title": "Boat ride on the bund #shanghai",
            "ispublic": 1,
            "isfriend": 0,
            "isfamily": 0 },
          { "id": "22224646391",
    ...
    search_response = { "photos": {
      ...
      "photo": [
            { "id": "22026530338",
              ...
              "server": "612",
              "farm": 1,
              "title": "books and coffee, is there a better match? #cafeparis",
              "ispublic": 1,
              "isfriend": 0,
              "isfamily": 0 },
            { "id": "22027077879",
    ...
    stub_request(:get, "https://api.flickr.com/services/rest?\
      api_key=#{ENV['FLICKR_API_KEY']}&extras=media&format=json&\
      method=flickr.photos.getRecent&nojsoncallback=1&per_page=10").
        to_return(status: 200, body: "#{recent_response}", headers: {})

    stub_request(:get, "https://api.flickr.com/services/rest?\
      api_key=#{ENV['FLICKR_API_KEY']}&extras=media&format=json&\
      media=photos&method=flickr.photos.search&nojsoncallback=1&\
      per_page=10&text=coffee").
