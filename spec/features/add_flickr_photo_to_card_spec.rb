...
describe "Add Flickr photo to card" do
  ...
  xdescribe 'when user visits new card page', js: true do
  ...
  xdescribe 'when user clicks Try Flickr button', js: true do
    before do
  ...
  xdescribe 'when user clicks GO button for searching photos', js: true do
    before do
      page.find("#flickr_button").click
      click_button 'GO'
    end
  ...
  xdescribe 'when user selects the photo to attach to the card', js: true do
    before do
      page.find("#flickr_button").click
      click_button 'GO'
      wait_for_ajax
    ...
    it "highlights the selected photo" do
      photo = page.find("#thumbnail_0") 
      photo.click
      expect(photo).to have_css("a.thumbnail.thumbnail_photo.selected")
    end
    it "fill the hidden remote_image_url field with proper url"
  end

  xdescribe 'when user clicks button for saving new card with photo' do
    before do
